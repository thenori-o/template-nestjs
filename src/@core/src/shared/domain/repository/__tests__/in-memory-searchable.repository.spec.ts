...
describe("InMemorySearchableRepository Unit Tests", () => {
  ...
  describe("applyFilter method", () => {
    it("shouldn't filter items when filter param is null", async () => {
  ...
  describe("applySort method", () => {
    ...
    it("should sort items", async () => {
      ...
      expect(sortedItems).toStrictEqual([items[2], items[0], items[1], items[3]]);

      sortedItems = await repository["applySort"](items, "name", "desc");
      expect(sortedItems).toStrictEqual([items[3], items[0], items[1], items[2]]);
    });
  });
  describe("applyPagination method", () => {
    it("should paginate items", async () => {
      ...
      let sortedItems = await repository["applyPagination"](items, 1, 2);
      expect(sortedItems).toStrictEqual([items[0], items[1]]);

      sortedItems = await repository["applyPagination"](items, 2, 2);
      expect(sortedItems).toStrictEqual([items[2], items[3]]);

      sortedItems = await repository["applyPagination"](items, 3, 2);
      expect(sortedItems).toStrictEqual([items[4]]);

      sortedItems = await repository["applyPagination"](items, 4, 2);
